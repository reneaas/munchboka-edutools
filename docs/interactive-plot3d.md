...
[`backend: threejs`](interactive-plot3d-threejs.md) is now the default and
gives live browser rotation, zoom, and geometry. This page documents the
legacy `frames` backend, kept for existing figures via an explicit
`backend: frames`.
...
`interactive-plot3d` supports the same drawing and macro keys as `plot3d-2`: `angle`, `curve`, `line`, `line-segment`, `ngon`, `normal-segment`, `point`, `plane`, `prism`, `pyramid`, `right-angle`, `sphere`, `solid-of-revolution`, `text`, `vector`, `let`, `def`, `repeat`, `macro`, `use`, and `endmacro`.
